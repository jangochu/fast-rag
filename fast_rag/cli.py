...
def default_index_dir(cfg: Config) -> Path:
    """Per-provider index subdir so switching provider never poisons the index."""
    return Path("index") / cfg.provider


def _resolve_index_dir(arg_value: Path | None, cfg: Config) -> Path:
    return arg_value if arg_value is not None else default_index_dir(cfg)


def cmd_ingest(args: argparse.Namespace) -> int:
    cfg = Config.from_env()
    index_dir = _resolve_index_dir(args.index_dir, cfg)
    print(f"[provider={cfg.provider}] Building index from {args.data_dir} → {index_dir} ...")
    if args.rebuild:
    ...
    try:
        ...
        # E.g. provider=bailian without DASHSCOPE_API_KEY.
...
def cmd_ask(args: argparse.Namespace) -> int:
    ...
    if not index_dir.exists():
        print(
            f"索引未找到 ({index_dir}),请先运行 'python -m fast_rag.cli ingest'",
            file=sys.stderr,
        )
        return 2

    try:
        result = answer(args.question, index_dir, k=args.k, config=cfg)
    except ValueError as e:
        # E.g. provider=bailian without DASHSCOPE_API_KEY.
        print(str(e), file=sys.stderr)
        return 5
        ...
        if cfg.provider == "ollama":
            if any(token in msg for token in ("connection refused", "connecterror", "max retries", "failed to connect")):
                print(
                    f"无法连接 Ollama ({cfg.ollama.url}),请先运行 'ollama serve'",
                    file=sys.stderr,
                )
                return 3
            if "model" in msg and any(token in msg for token in ("not found", "does not exist", "try pulling")):
                print(
                    f"模型 '{cfg.ollama.llm_model}' 不存在,请运行 'ollama pull {cfg.ollama.llm_model}'",
                    file=sys.stderr,
                )
                return 4
        if cfg.provider == "bailian":
            print(f"百炼 API 调用失败: {e}", file=sys.stderr)
            return 6
        raise
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="fast-rag",
        description="A simple, local-or-remote RAG demo (FAISS + Ollama/Bailian).",
    )
    ...
    p_ingest.add_argument(
        ...
        type=Path,
        default=None,
        help="Override default index dir (default: index/<provider>/)",
    )
    p_ingest.add_argument(
